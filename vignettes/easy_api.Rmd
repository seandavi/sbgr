...
## Cheatsheet
This cheatsheet provide main API designed for end users, please note "single" 
...
For a searching function that need name or id, when both are empty, they will return every object (file, project, pipeline, task, memeber) in a list. These function usually has opetional parameter called `ignore.case` and `exact` used 
for matching, when `exact = FALSE` you can use pattern to grep objects.

The cheatsheet only show some most used arguments, please check their help function to read more details.
...
So you can do a cascading like

```{r}
a$project("API")$file("sample1.tgz")$download("~/Desktop/")
```

Plesase play the main function yourself, we are not covering all the function in this tutorial. Also some users may notice the object actually has lower level API binding as well for example, you can call `a$pipeline_list_project()` but those are used intenally not supposed to be used by end users. user can just call `a$prject("public")`
...
## Billing group
Billing group speicfy how your computation charges, when you first registered, you should have some free credit, with billing group called something like "FreeAccount". Or maybe your company/organization purchased a license, you should have other billing group attached as well 
...
To save your typing/lines for casading style, I want to save the project I am gonna use for this tutorial to a Project object first.    
...
a <- Auth("aef7e9e3f6c54fb1b338ac4ecddf1a56")
#new remove soon
token <- "9ba2f6296ea34de9a33c9f2f0b546f46"
a <- Auth(token)
## get billing info
b <- a$billing()
## create project
